...
class DoubleConv(nn.Module):
	...
	def __init__(self, in_channels, out_channels, kernel_size = 3, mid_channels=None):
		...
		self.double_conv = nn.Sequential(
			...
			nn.GELU(),
			nn.Conv2d(mid_channels, out_channels, kernel_size= 1, padding=0, dilation=1),
...
class DynConvolution(nn.Module):
	...
	def forward(self, x, dyn_weight, chunk_size=9192):
		"""
...
class DynConvolutionV2(nn.Module):
	def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, dilation=1, padding=1, groups=1, factor=64, downsample=False):
		...
		if downsample:
			self.compress = nn.Sequential(
                nn.MaxPool2d(2, 2),
				nn.Conv2d(in_channels, factor, 1, 1),
				nn.GroupNorm(32, factor),
				# nn.BatchNorm2d(factor),
				nn.GELU()
	...
	def __init__(
		...
		if self.ns >= 1:
			for d_rate, k_size in zip(dilation_rates, conv_sizes):
				...
				self.dyn_convs[str(k_size)] = nn.ModuleList([
					DynConvolutionV2(in_channels=mid_channels, out_channels=mid_channels, kernel_size=k_size, padding=kernel_size // 2, 
					dilation=d_rate, downsample=True if factor > 1 else False) for _ in range(conv_levels)
				])
	...
	def __init__(
		...
		if self.ns >= 1:
			for d_rate, k_size in zip(dilation_rates, conv_sizes):
				...
				self.layers_pre[str(k_size)] = nn.ModuleList([
					DoubleConvLS(mid_channels // factorw, mid_channels, 1, 1, 0, 1) for _ in range(conv_levels)
				])

				self.layers_post[str(k_size)] = nn.ModuleList([
					DoubleConvLS(mid_channels, mid_channels // factorw, 1, 1, 0, 1) for _ in range(conv_levels)
				])
